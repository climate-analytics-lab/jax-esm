...
``python -m jem.main +configuration=veros-double-drake``
    ...
    grid, sharing the atmosphere's own grid.
...
:mod:`jem.components.veros.setups` (:mod:`~jem.components.veros.setups.
double_drake` / :mod:`~jem.components.veros.setups.earth`), and neither has a
land or sea-ice component -- see each configuration's own comment for what
that means for the atmosphere's land boundary conditions.

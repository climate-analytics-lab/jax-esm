...
def days_of_year_in_date(dt: jdt.Datetime):
    ...
    next_year = datetime(pydt.year + 1, 1, 1, tzinfo=UTC)
    this_year = datetime(pydt.year, 1, 1, tzinfo=UTC)
...
def days_of_month_in_date(dt: jdt.Datetime):
    ...
    next_month = datetime(year_of_next_month, pydt.month % 12 + 1, 1, tzinfo=UTC)
    this_month = datetime(pydt.year, pydt.month, 1, tzinfo=UTC)

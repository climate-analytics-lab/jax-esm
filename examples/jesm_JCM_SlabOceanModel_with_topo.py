...
if __name__ == "__main__":
    ...
    JCM_topography_file = (
        Path(jcm.__file__).parent / "data/bc/boundaries_daily_t31.nc"
    ).resolve()
    ...
    if SlabOceanModel_grid_specification[:5] == "JCM::":
        ...
        SlabOceanModel_mask_file = (
            Path(jax_esm.__file__).parent / "data/veros/veros_4deg.nc"
        ).resolve()
    ...
    for rel_year in range(simulate_years):
        _end_datetime = _start_datetime + jdt.to_timedelta(
            days_of_year_in_date(_start_datetime), "day"
        )
        ...
        for component_name, ds in output_dict.items():
            output_file = (
                output_dir / f"JESM-JCM_SOM-{component_name:s}-{current_year:04d}.nc"
            )

"""Example of coupling of simple slab atmosphere, ocean, and SPEEDY land models."""


def test_integration():

    from jax_esm.components import SlabOceanModel, SlabAtmosphereModel, LandModel
    ...
    output_dir = Path("output/SAM_SOM_LM").resolve()
    ...
    components = dict(
        ...
        lnd=LandModel(
            ...
            tdland=40.0,  # 40 days dissipation timescale (SPEEDY default)
            depth_soil=1.0,  # 1m soil depth (SPEEDY default)
            depth_lice=5.0,  # 5m ice depth (SPEEDY default)
        ),
    )

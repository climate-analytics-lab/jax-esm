...
def couple_atm_ocn(
    ...
    forcing_mapper.add_forcing_mapping(
        ...
        "atm",
        {
            "prog.sea_surface_temperature": "scalar.sea_surface_temperature",
        },
    )
    ...
    forcing_mapper.add_transformation(
        ...
        "ocn",
        surface_flux_name,
        "flux.total_heat_flux",
        interpolators["atm_to_ocn_hfluxn"],
    )
...
def couple_atm_ocn_lnd(
    ...
    forcing_mapper.add_transformation("ocn", "atm", "prog.sea_surface_temperature", "scalar.sea_surface_temperature", interpolators["ocn_to_atm"])
...
def generate_atm_lnd_interpolators(
    domain : Dict[str, Domain],
):
    interpolators = {}

    atm_domain = domain["atm"]
    lnd_domain = domain["lnd"]

    if atm_domain.grid_specification.grid_type in ["JCM", "Veros"] and lnd_domain.grid_specification.grid_type in ["JCM", "Veros"]:
        ...
        atm_latitude, atm_longitude = find_latitude_longitude(atm_domain)
        lnd_latitude, lnd_longitude = find_latitude_longitude(lnd_domain)

        atm_latlon = atm_domain.grid_specification.grid_type == "Veros"
        lnd_latlon = lnd_domain.grid_specification.grid_type == "Veros"

        interpolator_atm_to_ocn = BilinearInterpolator(
            longitude_source_deg = atm_longitude,
            latitude_source_deg = atm_latitude,
            longitude_target_deg = lnd_longitude,
            latitude_target_deg = lnd_latitude,
            periodic_longitude = True,
            target_mask = None,
            source_mask = None,
            fill_value = 0.0,
        ...
        interpolator_lnd_to_atm = BilinearInterpolator(
            longitude_target_deg = atm_longitude,
            latitude_target_deg = atm_latitude,
            longitude_source_deg = lnd_longitude,
            latitude_source_deg = lnd_latitude,
            periodic_longitude = True,
            target_mask = None,
            source_mask = None,
            fill_value = 0.0,
        ...
        def atm_to_lnd_hfluxn(in_array):
            in_array = in_array.sum(axis=-1)
            return conditional_transpose(
        ...
        def lnd_to_atm(in_array):
            return conditional_transpose(
                interpolator_lnd_to_atm.apply_scalar(conditional_transpose(in_array, not lnd_latlon)),
                not atm_latlon
            )
        interpolators["lnd_to_atm"] = lnd_to_atm
        interpolators["atm_to_lnd_hfluxn"] = atm_to_lnd_hfluxn
    else:
        raise Exception("Currently only support grids in JCM and Veros.")
